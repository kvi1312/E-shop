class WebhooksController < ApplicationController
  ...
  def stripe
    ...
    endpoint_secret = Rails.application.credentials.dig(:stripe, :webhook_secret_key)
    event = nil
    ...
    begin
      ...
      puts "Webhook signature verification FAILED: #{error.message}"
      status 400
      return
    end
    ...
    when "checkout.session.completed"
      ...
      address = "#{shipping_details["address"]["line1"]} #{shipping_details["address2"]["city"]}, #{shipping_details["address"]["state"]} #{shipping_details["address"]["postal_code"]}"
